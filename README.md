...
> (*Settings → Keymap → gear ⚙️ → Export Keymap*), drop the `.xml` into
> `source/`, and run `./port.py --skip-resolve`. That path is pure Python and
> needs no IDE detection.
...
- 🔸 **~340** keymap actions with no VS Code command (tool windows, most
  ...
  these — on macOS to <kbd>Cmd</kbd>, not <kbd>Ctrl</kbd>. To pull one onto
  <kbd>Ctrl</kbd>, add it to `manualActionCommand` and regenerate.
- 🖱️ **~14** mouse shortcuts — `keybindings.json` cannot express mouse bindings.
...
| Key | This port | Note |
|---|---|---|
| <kbd>Ctrl</kbd>+<kbd>numpad +</kbd> / <kbd>-</kbd> | zoom in / out | fold-all loses the bare-numpad combo; **fold still works on <kbd>Ctrl</kbd>+<kbd>=</kbd> / <kbd>Ctrl</kbd>+<kbd>-</kbd>** |
...
- 🚫 Don't commit the per-user build outputs — they are git-ignored for a reason.
...
- 🗺️ **Mapping data** from [kasecato/vscode-intellij-idea-keybindings](https://github.com/kasecato/vscode-intellij-idea-keybindings)
